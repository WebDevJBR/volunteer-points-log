...
import classes from './EditVolunteer.module.scss';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (volunteer.kingdom) {
          localGroups = await ApiService.get<Array<ILocalGroup>>(
            ApiEndpoints.LocalGroups,
            {
              kingdomId: volunteer.kingdom
            }
          );
        }
  ...
  const submitHandler = async () => {
    ...
    if (
      ...
      try {
        ...
        setSnackbar('Updated volunteer successfully', 'success');
        history.push('/landing/user');
      } catch (error) {
        setSnackbar('An error occurred while updating volunteer', 'error');
      }
    }
  };
