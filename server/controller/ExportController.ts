...
import { getManager, Repository, Like, createQueryBuilder, AdvancedConsoleLogger } from "typeorm";
import * as json2csv from 'json2csv';
import * as fs from "fs";
import { HttpStatusCodes } from "../constants/HttpStatusCodes";
import { Kingdom, Volunteer, VolunteerTimeEntry, ToReceivedFundsType_REF, LocalGroup } from "../entity"
...
export class ExportController {
   ...
   */
  static async exportKingdomHours(request: any, response: Response): Promise<void> {
    
    const kingdomRepo: Repository<Kingdom> = getManager().getRepository(Kingdom);
   
    // Grab data from kingdom, vols, time entries
    let kingdomsWithVolunteerTime = await kingdomRepo
        .find({ where: {}, 
                join: {
                        alias: 'kingdom', 
                        leftJoinAndSelect: { 
   ...
   * @param response The HTTP response.
   */
  static async exportKingdomBreakdown(request: any, response: Response): Promise<void> {
    
    const kingdomRepo: Repository<Kingdom> = getManager().getRepository(Kingdom);
    const toReceieveRepo: Repository<ToReceivedFundsType_REF> = getManager().getRepository(ToReceivedFundsType_REF);
    ...
    let localGroupEntries = await localGroupRepo.find({
        where: { kingdom:  kingdomId, toReceiveFundsType: toReceiveFundsTypeRefLG},
        join: { 
            alias: 'localGroup', 
            leftJoinAndSelect: {
    ...
    localGroupEntries.forEach(lg => {
        ...
        csv_data.push({"Kingdom": lg.name, "Actual Hours": actual.toFixed(2), "Earned Hours": earned.toFixed(2)});
    });
    ...
    otherEntries.forEach(vol => {
        ...
        csv_data.push({"Kingdom": vol.other, "Actual Hours": actual.toFixed(2), "Earned Hours": earned.toFixed(2)});
    });
