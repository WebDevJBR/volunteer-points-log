...
export class ImportController {
   ...
   */
  static async importKingdomsAndGroups(
    request: any,
    response: Response
  ): Promise<void> {
  ...
  static async importVolunteers(
    ...
    const receieveFundsTypeRepo: Repository<ToReceiveFundsType_REF> = getManager().getRepository(
    ...
    fs.createReadStream(request.file.path)
      ...
      .on('data', async vol => {
        ...
        Object.keys(vol).map((key, index) => {
          vol[key] = typeof vol[key] === 'string' ? vol[key].trim() : vol[key];
        });

        // Required to include header row in csv file, but skips over it
        if (rows > 0) {
          let existingVolunteer: Volunteer = await volunteerRepo.findOne({
            name: vol.sca
          });
          let existingKingdom: Kingdom = await kingdomRepo.findOne({
            name: vol.kingdom
          ...
          let fundsType: ToReceiveFundsType_REF = await receieveFundsTypeRepo.findOne(
          ...
          newVolunteer.infoMissing =
            !existingKingdom || !existingGroup ? true : false;
          ...
          if (vol.toReceive) {
            if (vol.toReceive == vol.localGroup && existingGroup) {
              ...
              fundsType = await receieveFundsTypeRepo.findOne({
                where: { type: Like('Kingdom') }
              });
              newVolunteer.toReceiveFundsType = fundsType?.id;
            } else {
  ...
  static async importDepartments(
    ...
    const departmentRepo: Repository<Department> = getManager().getRepository(
      Department
    );
    const volunteerRepo: Repository<Volunteer> = getManager().getRepository(
      Volunteer
    ...
    fs.createReadStream(request.file.path)
      ...
      .on('data', async row => {
        ...
        // Required to include header row in csv file, but skips over it
        if (rows > 0) {
          let existingDepartment: Department = await departmentRepo.findOne({
            name: row.department
          });
          let exisitingHead: Volunteer = await volunteerRepo.findOne({
            name: row.head
          });
          let exisitingDeputy: Volunteer = await volunteerRepo.findOne({
            name: row.deputy
          });
          ...
          if (existingDepartment) {
            ...
            newDepartment.name = row.department
              ? row.department
              : newDepartment.name;
